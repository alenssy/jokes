...
export class Joke{
    ...
    render(){
        let liked = `<svg width="20" height="17" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        let unliked = `<svg width="20" height="17" viewBox="0 0 20 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        let time = this.joke.created_at.split('');
        time.length=19;

        let jokesBlock = document.querySelector(`#jokesBlock`);

        let cardJoke = document.createElement('div');
        cardJoke.dataset.id=this.joke.id;
        cardJoke.classList.add('card__joke');

        this.joke.liked && cardJoke.classList.add('card__joke--favourite');

        let cardFav = document.createElement('div');
        cardFav.classList.add('card__favourite');

        cardJoke.append(cardFav);
        ...
        btnFav.id = 'btnFav';
        btnFav.classList.add(`btn__like`);

        btnFav.innerHTML = this.joke.liked ? liked : unliked;
		btnFav.dataset.favourite = this.joke.liked ? true : false;
        btnFav.addEventListener('click', this.addFavourite.bind(this));

        cardFav.append(btnFav);
        
        let cardJokeMain = `<div class="message__logo">
                        ...
                        <div class="info__main">
                            <div class="link">
                                <p class="id">id: <a href="${this.joke.id}" class="link__text">${this.joke.id}</a></p>
                            ...
                            <div class="info__add">
                                <p class="update__time">Last update: <span>${time.join('')}</span></p>
                                ${this.joke.categories.length>0 && !this.joke.liked ? `<span class="category__name">${this.joke.categories}</span>` : ``}
                            </div>
                        </div>
                    `;

        cardBody.innerHTML += cardJokeMain;
        cardJoke.append(cardBody);

        if(this.joke.liked){
            favJokesBlock.append(cardJoke);
        }else{
            jokesBlock.append(cardJoke);
        }
    }
    ...
    addFavourite(){
        ...
        let jokeBtn = jokesBlock.querySelector(`.card__joke[data-id="${this.joke.id}"] button`);

        let favJokes = localStorage.getItem('favJokes');
        favJokes = favJokes ? JSON.parse(favJokes) : {};


        if(jokeBtn && jokeBtn.dataset.favourite==="false"){
			this.joke.liked = true;
			jokeBtn.innerHTML = liked;
			jokeBtn.dataset.favourite = true;
			favJokes[this.joke.id] = this;
		} else{
			if(jokeBtn){
				jokeBtn.innerHTML = unliked;
				jokeBtn.dataset.favourite = false;
			}
			this.joke.liked = false;
			delete favJokes[this.joke.id];
		}
